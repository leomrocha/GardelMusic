...
"""
the 'color_map' dict is an adaptation from JS to python from:
http://mudcu.be/midi-js/ MIDI.js library js/MusicTheory/Synesthesia.js
...
color_map = {
		'Isaac Newton (1704)': { 
			'format': "HSL",
			'ref': "Gerstner, p.167",
			'english': ['red',None,'orange',None,'yellow','green',None,'blue',None,'indigo',None,'violet'],
			0: [ 0, 96, 51 ], ## C
			1: [ 0, 0, 0 ], ## C#
			2: [ 29, 94, 52 ], ## D
			3: [ 0, 0, 0 ], ## D#
		...
		'George Field (1816)': { 
			'format': "HSL",
			'ref': 'Klein, p.69',
			'english': ['blue',None,'purple',None,'red','orange',None,'yellow',None,'yellow green',None,'green'],
			0: [ 248, 82, 28 ],
			1: [ 0, 0, 0 ],
			2: [ 302, 88, 26 ],
			3: [ 0, 0, 0 ],
		...
		'August Aeppli (1940)': { 
			'format': "HSL",
			'ref': 'Gerstner, p.169',
			'english': ['red',None,'orange',None,'yellow',None,'green','blue-green',None,'ultramarine blue','violet','purple'],
			0: [ 0, 96, 51 ],
			1: [ 0, 0, 0 ],
			2: [ 29, 94, 52 ],
			3: [ 0, 0, 0 ],
		...
		#commented because it breaks my schemes
...
def get_color(midi_id, color_theme='Steve Zieverink (2004)'):
    ...
    if  'format' in theme and theme['format'] == 'HSV':
        ...
        color = reformat_color(color)
    elif  'format' in theme and theme['format'] == 'HEX':
        h_color = theme['data'][midi_id]
        r = int(h_color[1:3], 16)
        g = int(h_color[3:5], 16)
        b = int(h_color[5:], 16)
        color = (r,g,b)
...
def set_color_theme(theme_name):
    """
    sets a new color theme and allows for fast search without transformation
    """
    current_color_theme_name = theme_name
    current_colors = default_colors = get_all_colors(theme_name)
    

def get_color_from_current(midi_id):
    """
    returns the color for the given midi id for the current color theme
    """
    return current_colors[midi_id]
